...
import { SCHOOL_PROFILE, NEWS_ARTICLES, FACILITIES } from '../constants';
import { Calendar, ChevronRight, Building, CheckCircle } from 'lucide-react';
import { NewsArticle } from '../types';

const HeroSection: React.FC = () => (
  <div className="relative bg-brand-blue text-white">
    <div className="absolute inset-0">
      <img className="w-full h-full object-cover" src="https://picsum.photos/seed/hero/1600/900" alt="Suasana Sekolah" />
    ...
    <div className="relative max-w-7xl mx-auto py-24 px-4 sm:py-32 sm:px-6 lg:px-8 text-center">
      ...
      <div className="mt-8 flex justify-center space-x-4">
        <Link
          to="/profil"
          className="inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-brand-blue bg-white hover:bg-gray-100"
        >
          Tentang Kami
        </Link>
        <Link
          to="/kontak"
          className="inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-brand-secondary hover:bg-amber-400"
        >
          Hubungi Kami
        </Link>
      </div>
    ...
    <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      <div className="grid md:grid-cols-3 gap-8 items-center">
        <div className="md:col-span-1">
          <img src={SCHOOL_PROFILE.welcome.imageUrl} alt={SCHOOL_PROFILE.welcome.headmasterName} className="rounded-full shadow-lg mx-auto w-48 h-48 md:w-64 md:h-64 object-cover" />
        </div>
        <div className="md:col-span-2">
          <h2 className="text-3xl font-bold text-gray-900">Sambutan Kepala Sekolah</h2>
          <p className="text-lg font-medium text-brand-blue mt-1">{SCHOOL_PROFILE.welcome.headmasterName}</p>
          <p className="mt-4 text-gray-600 leading-relaxed">
            {SCHOOL_PROFILE.welcome.message}
          </p>
        </div>
      </div>
    </div>
...
const FacilitiesPreview: React.FC = () => (
